...
function LoginAsCounsellor() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_SERVER_DOMAIN}/api/counsellor/login`, {
                method: "POST",
                headers: {
            ...
            console.log(resData.data.refreshToken);
            if (response.status === 200) {
                // Success: Navigate to another page (e.g., dashboard)
                toast.success("Counsellor logged in successfully.");
    ...
    return (
        <div className="bg-gradient-to-b from-purple-100 via-white to-purple-200  md:min-h-screen md:p-12 min-h-[600px]">
            <div className="bg-white p-4 w-96 rounded-md drop-shadow-md m-auto">
                <h1 className="font-serif text-2xl mb-4">Please Login As Counsellor</h1>
                ...
                {/* Links */}
                <div className='mt-3'>
                    <p>Yet not registered? <span className='text-blue-500 hover:text-blue-700 cursor-pointer' onClick={() => navigate("/registerCounsellor")}>Register</span></p>
...
export default LoginAsCounsellor;
